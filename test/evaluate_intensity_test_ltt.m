function yy = evaluate_intensity_test_ltt(Q,aa,bb,cc,occ,scale,En,extpar,modepar)
...
F = sum(occ(1)*repmat(aff_La_test_ltt(Q,aa,bb,cc),1,64) .* exp(-2*pi*1i * Q*transp(atomic_positions_La_test_ltt(modepar(1:14)))),2)+ ...
    ...
    sum(occ(6)*repmat(aff_Ba_test_ltt(Q,aa,bb,cc),1,64) .* exp(-2*pi*1i * Q*transp(atomic_positions_La_test_ltt(modepar(1:14)))),2);
